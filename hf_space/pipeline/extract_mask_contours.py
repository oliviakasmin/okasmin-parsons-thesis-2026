...
def load_mask_image(object_id, image_directory):
    mask_path = image_directory / f"{object_id}_mask_standardized.png"
    img = PImage.open(mask_path).convert("RGBA")
    return img


def extract_outline_from_mask(object_id, alpha_threshold=8, image_directory=test_images_dir):
    img = load_mask_image(object_id, image_directory)
